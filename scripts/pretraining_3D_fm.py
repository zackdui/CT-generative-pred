...
def main(training_args, 
    ...
    light_model = UnetLightning3D(DiffusionModelUNet, unet_kwargs, lr=lr, output_dir=run_output_dir, input_channels=unet_3d_cfm_configs.in_channels, decode_model=decode_model, img_size=training_args.img_size)
    ...
    if debug_flag:
        ...
        trainer = pl.Trainer(logger=[csv_logger, wandb_logger],
                            ...
                            # profiler=AdvancedProfiler(dirpath=".", filename="pl_profiler.txt"), #"advanced",
...
def create_datasets(base_paths, max_examples, encode_model=None, encoded_data=False):
...
if __name__ == "__main__":
    ...
    # if the checkpoint is None but encoded_data is True it will used the cached encoded data
    encode_model_checkpoint = None
    ...
    if encode_model_checkpoint is not None:
        model = CustomVAE.load_from_checkpoint(encode_model_checkpoint, map_location="cuda")
        decode_model = model
    elif encode_data:
    ...
    train_dataset, val_dataset, test_dataset = create_datasets(base_paths, num_samples, model, encoded_data=encode_data)

    main(training_args, unet_3d_cfm, train_dataset, val_dataset, test_dataset, run_output_dir, decode_model=decode_model, single_image=single_image)
