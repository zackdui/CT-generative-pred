...
if __name__=="__main__":
    ...
    evaluator_prep_interp = ImageEvaluatorPrep(vae_model, lit, n_steps=50, reverse_input=False, interpolate=True)
    evaluator_prep_no_interp = ImageEvaluatorPrep(vae_model, lit, n_steps=50, reverse_input=False, interpolate=False)
    ...
    with open("test_raw_data_nodule_original_boxes.json", "r") as f:
    ...
    for batch in test_loader:
        ...
        preped_input_images = evaluator_prep_no_interp.prepare_images_for_eval(input_images, sample=False)
        preped_input_images_inter = evaluator_prep_interp.prepare_images_for_eval(input_images, sample=False)

        preped_output_images =  evaluator_prep_no_interp.prepare_images_for_eval(output_images, sample=False)
        preped_output_images_inter = evaluator_prep_interp.prepare_images_for_eval(output_images, sample=False)
        ...
        for i in range(len(meta)):
            ...
            assert output_data[nodule_id_a]['bbox'] == original_outputs[nodule_id_a]['bbox'], f"Mismatch in bbox for nodule {nodule_id_a}"
            ...
            nodule_differences.append(abs(output_data[nodule_id_a]['nodule_volume'] - original_outputs[nodule_id_a]['nodule_volume']))
            nodule_differences.append(abs(output_data[nodule_id_b]['nodule_volume'] - original_outputs[nodule_id_b]['nodule_volume']))

            if saved_examples < max_saved_examples:
                if saved_examples == 0:
                    print("Print saving first example!!")
                save_montage_with_bbox3d(preped_input_images[i], out_path=f"./test_encoded_eval_segmentations/{saved_examples}_montage_in.png", bbox_yxz=small_bbox_input, title="Input Image with Nodule BBox No Interpolation")
                input_fig = save_montage_with_bbox3d(preped_input_images_inter[i], out_path=f"./test_encoded_eval_segmentations/{saved_examples}_montage_in_inter.png", bbox_yxz=small_bbox_input_interp, title="Input Image with Interpolation", return_fig=True)
                binary_seg_fig = save_montage(binary_segmentation_input.float()[i], out_path=f"./test_encoded_eval_segmentations/{saved_examples}_montage_in_seg.png", title="Input Segmentation", return_fig=True)
                save_two_figs_side_by_side(input_fig, binary_seg_fig, out_path=f"./test_encoded_eval_segmentations/{saved_examples}_montage_in_combined_vol_{volumes_in[i].item()}.png")
                # import pdb; pdb.set_trace()
                plt.close(input_fig)
                plt.close(binary_seg_fig)

                save_montage_with_bbox3d(preped_output_images[i], out_path=f"./test_encoded_eval_segmentations/{saved_examples}_montage_out.png", bbox_yxz=small_bbox_output, title="Output Image with Nodule BBox No Interpolation")
                output_fig = save_montage_with_bbox3d(preped_output_images_inter[i], out_path=f"./test_encoded_eval_segmentations/{saved_examples}_montage_out_inter.png", bbox_yxz=small_bbox_output_interp, title="Output Image with Interpolation", return_fig=True)
                binary_seg_fig_out = save_montage(binary_segmentation_output.float()[i], out_path=f"./test_encoded_eval_segmentations/{saved_examples}_montage_out_seg.png", title="Output Segmentation", return_fig=True)
                save_two_figs_side_by_side(output_fig, binary_seg_fig_out, out_path=f"./test_encoded_eval_segmentations/{saved_examples}_montage_out_combined_vol_{volumes_out[i].item()}.png")
            ...
            total += 1
            if total % 200 == 0:
                print(nodule_differences[total - 100:total - 90])
                print(f"Processed {total} nodules...")

    with open("test_encoded_data_nodule_original_boxes.json", "w") as f:
        json.dump(output_data, f)
    with open("nodule_differences.json", "w") as f:
        json.dump(nodule_differences, f)
    print(f"Average difference in the nodules is {sum(nodule_differences) / len(nodule_differences) if nodule_differences else 0}")

    file = "test_raw_data_nodule_original_boxes.json"
